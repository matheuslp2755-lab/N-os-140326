...
const MusicTrimmer: React.FC<MusicTrimmerProps> = ({ track, onConfirm, onBack }) => {
  ...
  useEffect(() => {
    if (audioRef.current && !isDragging) {
        ...
        audioRef.current.play().catch((e) => {
            console.warn("Autoplay blocked in trimmer:", e);
            setIsPlaying(false);
        });
    }
  }, [startTime, isDragging, track.previewUrl]);
  ...
  return (
    <div className="flex flex-col items-center gap-6 p-6 h-full bg-white dark:bg-black animate-fade-in overflow-hidden">
      ...
      <div className="relative mt-2 cursor-pointer group" onClick={togglePlay}>
        ...
        <div className="absolute -bottom-4 left-1/2 -translate-x-1/2 bg-sky-500 px-4 py-1.5 rounded-full shadow-xl border-4 border-white dark:border-black animate-pulse group-hover:scale-110 transition-transform">
            {isPlaying ? (
