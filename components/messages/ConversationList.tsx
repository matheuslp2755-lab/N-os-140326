...
const ConversationList: React.FC<ConversationListProps> = ({ onSelectConversation, currentUser }) => {
    const { t } = useLanguage();
        ...
        const unsubConvos = onSnapshot(q, (snapshot) => {
            ...
            Promise.all(conversationsPromises).then(resolvedConversations => {
                ...
                validConvos.sort((a, b) => {
                    const tsA = a.timestamp?.seconds || Date.now() / 1000;
                    const tsB = b.timestamp?.seconds || Date.now() / 1000;
                    return tsB - tsA;
                });
                ...
                uniqueUserIds.forEach(userId => {
                    ...
                    }, (err) => console.error(`Error watching user ${userId}:`, err));
