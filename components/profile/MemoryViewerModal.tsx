                ...
                <div className="flex items-center gap-2 mb-2">
                    {items.map((_, index) => (
                        <div key={index} className="flex-1 h-1 bg-white/30 rounded-full overflow-hidden">
                           <div ref={index === currentIndex ? progressRef : null} className={`h-full bg-white rounded-full ${index === currentIndex && !isPaused ? 'animate-progress' : ''}`} style={{ width: index < currentIndex ? '100%' : '0', animationDuration: isVideo ? `${(videoRef.current?.duration && !isNaN(videoRef.current.duration)) ? videoRef.current.duration : 5}s` : '5s' }} />
                        </div>
                    ))}
                </div>
